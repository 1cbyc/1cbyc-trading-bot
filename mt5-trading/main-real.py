#!/usr/bin/env python3
"""
MT5 Real Trading Bot for Deriv Synthetic Indices
WARNING: This script trades with REAL MONEY!
Use with extreme caution and only with money you can afford to lose.
"""
...
from real_config import MT5RealConfig  # Use real config instead of demo config
...
class MT5RealTradingBot:
    """MT5 Real Trading Bot for Deriv Synthetic Indices - REAL MONEY TRADING"""
    
    def __init__(self, symbols: Optional[List[str]] = None, strategy_type: str = 'multi'):
        # Use REAL configuration for symbols and strategy
        if symbols is None:
            self.symbols = MT5RealConfig.get_symbols_from_env()
        else:
    ...
    def _initialize_strategies(self):
        """Initialize strategies for each symbol"""
        strategy_class_name = MT5RealConfig.get_strategy_class(self.strategy_type)
        
        for symbol in self.symbols:
            try:
                # Get the strategy class dynamically
                strategy_class = globals()[strategy_class_name]
                self.strategies[symbol] = strategy_class(symbol, MT5RealConfig.TIMEFRAME)
                ...
                self.strategies[symbol] = MultiStrategy(symbol, MT5RealConfig.TIMEFRAME)
    ...
    def connect(self):
        """Connect to Deriv MT5 REAL ACCOUNT"""
        try:
            # Initialize MT5
            ...
            # Login to Deriv MT5 REAL account using config
            if not mt5.login(login=MT5RealConfig.MT5_LOGIN, password=MT5RealConfig.MT5_PASSWORD, server=MT5RealConfig.MT5_SERVER):
                print(f"❌ MT5 login failed: {mt5.last_error()}")
                return False
            
            print(f"✅ Logged in to Deriv MT5 REAL account: {MT5RealConfig.MT5_LOGIN}")
            ...
            if account_info:
                ...
                # WARNING for real trading
                print("⚠️  WARNING: This is a REAL MONEY trading account!")
                print("⚠️  Only trade with money you can afford to lose!")
                
                # Ask for confirmation
                response = input("Do you want to continue with real trading? (yes/no): ")
                if response.lower() != 'yes':
                    print("🛑 Real trading cancelled by user")
                    return False
            
            self.connected = True
            print("✅ Connected to Deriv MT5 REAL account successfully!")
            return True
    ...
    def calculate_signals(self, symbol: str, df: pd.DataFrame) -> Tuple[str, float]:
        ...
        try:
            ...
            strategy_weight = MT5RealConfig.get_strategy_weight(strategy.__class__.__name__)
    ...
    def check_and_manage_positions(self):
        ...
        try:
            ...
            for position in positions:
                ...
                if MT5RealConfig.ENABLE_DYNAMIC_PROFIT_TAKING and profit_percent > 0:
                    for level in MT5RealConfig.PROFIT_TAKING_LEVELS:
                ...
                # Trailing stop
                if MT5RealConfig.ENABLE_TRAILING_STOP and profit_percent >= MT5RealConfig.TRAILING_START_PERCENT:
    ...
    def update_trailing_stop(self, position, profit_percent: float):
        ...
        try:
            ...
            if position.type == mt5.ORDER_TYPE_BUY:
                new_sl = price_current * (1 - MT5RealConfig.TRAILING_STOP_PERCENT / 100)
    ...
    def process_symbol(self, symbol: str):
        ...
        try:
            ...
            # Get historical data
            df = self.get_historical_data(symbol, MT5RealConfig.TIMEFRAME, MT5RealConfig.DATA_LOOKBACK)
            if df.empty:
                return
            ...
            if signal != "HOLD" and confidence > MT5RealConfig.CONFIDENCE_THRESHOLD:
                print(f"🎯 Executing trade: {symbol} {signal} (confidence: {confidence:.2f})")
                
                # Use the correct minimum volume for this symbol
                position_size = MT5RealConfig.get_min_volume(symbol)
                ...
                sl = 0.0
                tp = 0.0
                if MT5RealConfig.STOP_LOSS_PERCENT > 0:
                    sl = current_price * (1 - MT5RealConfig.STOP_LOSS_PERCENT/100) if signal == "BUY" else current_price * (1 + MT5RealConfig.STOP_LOSS_PERCENT/100)
                if MT5RealConfig.TAKE_PROFIT_PERCENT > 0:
                    tp = current_price * (1 + MT5RealConfig.TAKE_PROFIT_PERCENT/100) if signal == "BUY" else current_price * (1 - MT5RealConfig.TAKE_PROFIT_PERCENT/100)
    ...
    def trading_loop(self):
        """Main trading loop"""
        print("🔄 Starting MT5 REAL trading loop...")
        ...
        while self.running:
            try:
                ...
                print(f"⏳ Waiting {MT5RealConfig.TRADING_INTERVAL} seconds before next iteration...")
                time.sleep(MT5RealConfig.TRADING_INTERVAL)
    ...
    def start(self):
        ...
        try:
            print("🚀 Starting MT5 Real Trading Bot...")
            
            # Validate configuration
            MT5RealConfig.validate_config()
    ...
    def stop(self):
        ...
        print("🛑 Stopping MT5 Real Trading Bot...")
        self.running = False
        
        # Close all positions (optional - uncomment if you want to close all positions on shutdown)
...
def main():
    """Main entry point"""
    print("🚀 MT5 Real Trading Bot for Deriv Synthetic Indices")
    print("⚠️  WARNING: This bot trades with REAL MONEY!")
    print("⚠️  Only use with money you can afford to lose!")
    print("=" * 60)
    
    # Create and start the bot
    bot = MT5RealTradingBot()
    bot.start()
